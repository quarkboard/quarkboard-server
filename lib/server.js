...
class Server extends Plugin {
    ...
    init() {
        ...
        this.app.on('plugin-loading', (plugin, opts) => {
            if (this.is(plugin)) {
                ...
                opts.push(['', 'no-server', 'Disable the use of the back-end server'])
        ...
        this.app.on('plugin-loaded', (plugin) => {
            if (this.is(plugin)) {
                const opts = Object.assign({
                    ...
                    noServer: this.app.options['no-server'],
                });
                ...
                this._server.listen(opts.port, opts.hostname, () => {
                    this.app.info(`Back-end listening on http${opts.https ? 's' : ''}://${opts.hostname}:${opts.port}/`);
